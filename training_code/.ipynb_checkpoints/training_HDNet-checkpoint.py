...
pre_ck_pnts_dir = "../training_progress/model/DepthEstimator"
model_num = '500'
model_num_int = 500
...
with refineNet_graph.as_default():
    ...
    ## ****************************tiktok****************************
    x1_tk = tf.compat.v1.placeholder(tf.float32, shape=(None, 256,256,9))
    n1_tk = tf.compat.v1.placeholder(tf.float32, shape=(None, 256,256,3))
    z1_tk = tf.compat.v1.placeholder(tf.bool, shape=(None, 256,256,1))
    
    x2_tk = tf.compat.v1.placeholder(tf.float32, shape=(None, 256,256,9))
    n2_tk = tf.compat.v1.placeholder(tf.float32, shape=(None, 256,256,3))
    z2_tk = tf.compat.v1.placeholder(tf.bool, shape=(None, 256,256,1))
    
    i_r1_c1_r2_c2 = tf.compat.v1.placeholder(tf.int32, shape=(None, 25000,5))
    i_limit = tf.compat.v1.placeholder(tf.int32, shape=(None, 24,3))
    ...
    ## ****************************optimizer****************************
    train_step = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001,
                                        beta1=0.9,
                                        beta2=0.999,
                                        epsilon=0.1,
                                        use_locking=False,
...
with sess.as_default():
    with refineNet_graph.as_default():
        ...
        saver = tf.compat.v1.train.import_meta_graph(pre_ck_pnts_dir+'/model_'+model_num+'/model_'+model_num+'.ckpt.meta')
