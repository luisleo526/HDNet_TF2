...
def depth2points3D_MB(output, Rt, Ki, cen, z_r, origin, scaling):
    ...
    indices = tf.compat.v1.where(ones_mat) #  (256*256*B) x 3 = N x 3
#     indices = tf.where(z_r)
    
    good_output = tf.compat.v1.where(z_r, output, 1000000*tf.ones_like(output))
    Dlambda = tf.gather_nd(good_output,indices) # N x 3

    num_of_points = tf.shape(input=Dlambda)[0] #N
    num_of_batches = tf.shape(input=z_r)[0]
...
# dividing xy and the batch number    
    bxy = idx # N x 3
    b = tf.transpose(a=bxy[...,0]) # 1 x N
    batches = tf.reshape(b,[1,num_of_points]) # 1 x N
    xy = tf.transpose(a=tf.reverse(bxy[...,1:3],[1])) # 2 x N
...
def dmap_to_nmap(Y, Rt, R, Ki, cen, Z, origin, scaling):
    BATCH_SIZE = tf.shape(input=Y)[0]
    IMAGE_HEIGHT = tf.shape(input=Y)[1]
    ...
    p3d_map1 = tf.compat.v1.where(Z,p3d_map1,tf.zeros_like(p3d_map1))
    p3d_map2 = tf.reshape(p3d[1,...],[BATCH_SIZE,IMAGE_HEIGHT,IMAGE_HEIGHT,1])
    p3d_map2 = tf.compat.v1.where(Z,p3d_map2,tf.zeros_like(p3d_map2))
    p3d_map3 = tf.reshape(p3d[2,...],[BATCH_SIZE,IMAGE_HEIGHT,IMAGE_HEIGHT,1])
    p3d_map3 = tf.compat.v1.where(Z,p3d_map3,tf.zeros_like(p3d_map3))
    ...
    output_no0 = tf.compat.v1.where(output_mask, 1e-5*tf.ones_like(n), n)
