...
def conv_layer(input_tensor,name,kernel_size,output_channels,initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0),stride=1,bn=False,training=False,relu=True):
	input_channels = input_tensor.get_shape().as_list()[-1]
	with tf.compat.v1.variable_scope(name) as scope:
		kernel = variable('weights', [kernel_size, kernel_size, input_channels, output_channels], initializer, regularizer=tf.keras.regularizers.l2(0.5 * (0.0005)))
		conv = tf.nn.conv2d(input=input_tensor, filters=kernel, strides=[1, stride, stride, 1], padding='SAME')
		biases = variable('biases', [output_channels], tf.compat.v1.constant_initializer(0.0))
...
def max_pooling(input_tensor,name,factor=2):
	pool = tf.nn.max_pool2d(input=input_tensor, ksize=[1, factor, factor, 1], strides=[1, factor, factor, 1], padding='SAME', name=name)
...
def batch_norm_layer(input_tensor,scope,training):
	return tf.compat.v1.layers.batch_normalization(input_tensor,training=training)
