...
def save_prediction_png (image,imagen,X,Z,Z3,Vis_dir,i,idx,perc):
    ...
    d = np.array(imageio.imread(Vis_dir+data_name+"_depth.png"),dtype='f')
    d = np.where(Z3[0,...]>0,d[...,0:3],255.0)
    n = np.array(imageio.imread(Vis_dir+data_name+"_normal.png"),dtype='f')
...
def read_renderpeople(rp_path, frms, IMAGE_HEIGHT,IMAGE_WIDTH):
    ...
    for b in range(Bsize):
        cur_f = int(frms[b])
        name = "%07d" %(cur_f)
        batch_densepose.append(imageio.imread(rp_path +'/densepose/'+name+'.png'))
        batch_color.append(imageio.imread(rp_path +'/color_WO_bg/'+name+'.png'))
        batch_mask.append(imageio.imread(rp_path +'/mask/'+name+'.png'))
        batch_depth.append(np.genfromtxt(rp_path +'/depth/'+name+'.txt',delimiter=","))
        cur_normal = np.array(imageio.imread(rp_path +'/normal_png/'+name+'.png'),dtype='f');
...
def read_tiktok(tk_path, frms, IMAGE_HEIGHT,IMAGE_WIDTH):
    ...
    for b in range(Bsize):
        cur_f = int(frms[b])
        name = "%07d" %(cur_f)
        batch_densepose.append(imageio.imread(tk_path +'/densepose/'+name+'.png'))
        batch_color.append(imageio.imread(tk_path +'/color_WO_bg/'+name+'.png'))
        batch_mask.append(imageio.imread(tk_path +'/mask/'+name+'.png'))
        cur_normal = np.array(imageio.imread(tk_path +'/pred_normals_png/'+name+'.png'),dtype='f');
