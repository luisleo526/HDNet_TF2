...
def rigid_transform_3D(A,B):
    A = tf.transpose(a=A) #3*N
    B = tf.transpose(a=B) #3*N
    num_rows = tf.shape(input=B)[0] #3
    num_cols = tf.shape(input=B)[1] #N
    centroid_A = tf.reshape(tf.reduce_mean(input_tensor=A,axis=1),[3,1]) #3*1
    centroid_B = tf.reshape(tf.reduce_mean(input_tensor=B,axis=1),[3,1]) #3*1
...
def Depth2Points3D_transformed_vector(Dlambda, indices , Rt, Ki, cen, origin, scaling):
    ...
    num_of_points = tf.shape(input=Dlambda)[0] #N
    num_of_batches = 1
...
# dividing xy and the batch number    
    bxy = idx # N x 3
    xy = tf.transpose(a=tf.reverse(bxy,[1])) # 2 x N
...
def transform_depth_PCs_dp_based2(C,R,Rt,cen,K,Ki,origin,scaling,d_i,d_j,i_r1_c1_r2_c2,i_limit):
    ...
    n = tf.shape(input=i_r1_c1_r2_c2)[0]
...
def reproject(point3D, K,R,C):
    ...
    point3D = tf.transpose(a=point3D)
    
    num_of_points = tf.shape(input=point3D)[1]
...
def compute_dp_tr_3d_2d_loss2(d_i,d_j,i_r1_c1_r2_c2,i_limit,C,R,Rt,cen,K,Ki,origin,scaling):
    ...
    err_vec = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(d),axis=1));
    loss3d = tf.reduce_mean(input_tensor=err_vec)
    ...
    err_vec = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(d),axis=1));
    loss2d = tf.reduce_mean(input_tensor=err_vec)
